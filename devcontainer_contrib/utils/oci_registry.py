...
class OCIRegistry:
    ...
    def parse_oci(oci_input: str) -> "OCIRegistry.ParsedOCIRef":
        ...
        return OCIRegistry.ParsedOCIRef(_id=_id,
                version=version,
                owner=owner,
                namespace=namespace,
                registry=registry,
    ...
    @staticmethod
    def _parse_www_authenticate(response_headers: Union[str, List[str], Dict[str,str]]) -> WWWAthenticate:

        if isinstance(response_headers, str):
            response_headers = response_headers.splitlines()
        
        elif isinstance(response_headers, dict):
            response_headers = [f"{key}: {value}" for key,value in response_headers.items()]
        ...
        for line in response_headers:
            ...
            if result is None:
                continue
            return OCIRegistry.WWWAthenticate(realm=result.group(1),
                                    service=result.group(2),
                                    scope=result.group(3))
        raise ValueError(f"failed to parse www-authenticate from the given string: {str(response_headers)}")

    @staticmethod
    def _generate_token(raw_response_header: str) -> str:
        www_authenticate = OCIRegistry._parse_www_authenticate(raw_response_header)
        response = urllib.request.urlopen(f"{www_authenticate.realm}?service={www_authenticate.service}&scope={www_authenticate.scope}")
        token = json.loads(response.read())['token']
        return token


    @staticmethod
    def _attempt_request(url: str, headers: Optional[Dict[str, str]] = None) -> http.client.HTTPResponse:
        if headers is None:
             headers= {}
    ...
    def get_blob(oci_input: str, digest: str) -> bytes:
        ...
        if not calculated_digest == digest:
            raise OCIRegistry.HashException(f"bad calculated digest: {calculated_digest} (expected {digest})")

...
class Invoker:
    ...
    def invoke(
        ...
        response = invoke.run(
            command, out_stream=sys.stdout, err_stream=sys.stderr, pty=True, warn=True, echo=True
        )

        if raise_on_failure and not response.ok:
            raise exception_class(command=command, error=f"Return Code: {response.return_code}")
